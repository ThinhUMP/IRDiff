...
if __name__ == "__main__":
    ...
    results_fn_list = sorted(
        results_fn_list, key=lambda x: int(os.path.basename(x)[:-3].split("_")[-1])
    )
    ...
    logger.info(
        f"Load generated data done! sample_id[{eval_start_index}:{eval_end_index}] examples for evaluation."
    )
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc="Eval")):
        ...
        all_pred_ligand_pos = r[
            "pred_ligand_pos_traj"
        ]  # [num_samples, num_steps, num_atoms, 3]
        ...
        for sample_idx, (pred_pos, pred_v) in enumerate(
            ...
            pred_atom_type = transforms.get_atomic_number_from_index(
                pred_v, mode=args.atom_enc_mode
            )
            ...
            pair_dist = eval_bond_length.pair_distance_from_pos_v(
                pred_pos, pred_atom_type
            )
            ...
            try:
                ...
                if args.verbose:
                    logger.warning(
                        "Reconstruct failed %s" % f"{example_idx}_{sample_idx}"
                    )
                continue
            n_recon_success += 1
            ...
            try:
                ...
                if args.verbose:
                    logger.warning(
                        "Evaluation failed for %s" % f"{example_idx}_{sample_idx}"
                    )
                continue
    ...
    success_pair_length_profile = eval_bond_length.get_pair_length_profile(
        success_pair_dist
    )
    success_js_metrics = eval_bond_length.eval_pair_length_profile(
    ...
    if args.save:
        eval_bond_length.plot_distance_hist(
            ...
            save_path=os.path.join(
                result_path,
                f"pair_dist_hist_{eval_start_index}-to-{eval_end_index}.png",
            ),
        )
    ...
    if args.save:
        torch.save(
            ...
            os.path.join(
                result_path,
                f"metrics_{args.eval_step}_{eval_start_index}-to-{eval_end_index}.pt",
            ),
        )
