...
def get_promt_dataset(config, *args, **kwargs):
    ...
    train_dataset = PromptSubset(
        dataset,
        indices=train_split_indices,
        prompt_indices=train_prompt_indices[:, -1].numpy().tolist(),
    )
    test_dataset = PromptSubset(
        dataset,
        indices=test_split_indices,
        prompt_indices=test_prompt_indices[:, -1].numpy().tolist(),
    )
...
def get_topk_promt_dataset(config, *args, **kwargs):
    ...
    train_dataset = TopKPromptSubset(
        dataset,
        indices=train_split_indices,
        prompt_indices=train_prompt_indices,
        topK=topk_prompt,
    )
    test_dataset = TopKPromptSubset(
        dataset,
        indices=test_split_indices,
