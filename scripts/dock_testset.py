...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dataset", type=str, default="./data/crossdocked_v1.1_rmsd1.0_pocket10"
    )
    parser.add_argument(
        "-s", "--split", type=str, default="./data/crossdocked_pocket10_pose_split.pt"
    )
    ...
    parser.add_argument(
        "--ligand_root", type=str, default="./data/crossdocked_v1.1_rmsd1.0_pocket10"
    )
    ...
    dataset, subsets = get_dataset(
        config=EasyDict({"name": "pl", "path": args.dataset, "split": args.split})
    )
    ...
    for i, data in enumerate(tqdm(test_set)):
        mol = next(
            iter(
                Chem.SDMolSupplier(os.path.join(args.ligand_root, data.ligand_filename))
            )
        )
        # try:
    ...
    if args.out is None:
        ...
        docked_name = (
            f"{split_name}_test_docked_uff_{args.use_uff}_size_{args.size_factor}.pt"
        )
