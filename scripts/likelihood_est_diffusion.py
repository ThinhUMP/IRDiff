...
def data_likelihood_estimation(model, data, time_steps, batch_size=1, device="cuda:0"):
    ...
    for i in range(num_batch):
        n_data = (
            batch_size
            if i < num_batch - 1
            else num_timesteps - batch_size * (num_batch - 1)
        )
    ...
    # prior
    batch = Batch.from_data_list(
        [data.clone() for _ in range(1)], follow_batch=FOLLOW_BATCH
    ).to(device)
    ...
    sum_kl_pos, sum_kl_v = model.num_timesteps * torch.mean(
        all_kl_pos
    ), model.num_timesteps * torch.mean(all_kl_v)
    all_kl_pos, all_kl_v = torch.cat([all_kl_pos, kl_pos_prior]), torch.cat(
        [all_kl_v, kl_v_prior]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/sampling/final_diffusion/aromatic_176k.yml",
    )
    parser.add_argument("--affinity_path", type=str, default="affinity_info.pkl")
    parser.add_argument(
        "--index_path",
        type=str,
        default="data/crossdocked_v1.1_rmsd1.0_pocket10/index.pkl",
    )
    ...
    ligand_featurizer = trans.FeaturizeLigandAtom(
        ckpt["config"].data.transform.ligand_atom_mode
    )
    transform_list = [
        protein_featurizer,
        ligand_featurizer,
    ...
    model.load_state_dict(
        ckpt["model"], strict=False if "train_config" in config.model else True
    )
    ...
    torch.save(
        valid_train_results, os.path.join(args.result_path, "crossdocked_train.pt")
    )
    torch.save(
        valid_test_results, os.path.join(args.result_path, "crossdocked_test.pt")
    )
