...
def eval_single_datapoint(index, id, args):
    ...
    results = []
    n_eval_success = 0
    for sample_idx, sample_dict in enumerate(
        tqdm(index[:num_samples], desc="Eval", total=num_samples)
    ):
        mol = sample_dict["mol"]
        ...
        try:
            ...
            if "vina" in sample_dict:
                ...
                if args.docking_mode == "qvina":
                    ...
                    vina_task = VinaDockingTask.from_generated_mol(
                        deepcopy(mol), ligand_filename, protein_root=args.protein_root
                    )
        ...
        results.append(
            {**sample_dict, "chem_results": chem_results, "vina": vina_results}
        )
    logger.info(
        f"Evaluate No {id} done! {num_samples} samples in total. {n_eval_success} eval success!"
    )
    torch.save(
        results,
        os.path.join(
            args.result_path,
            f"eval_{id:03d}_{os.path.basename(ligand_filename[:-4])}.pt",
        ),
    )
    return results
...
if __name__ == "__main__":
    ...
    with Pool(args.num_workers) as p:
        for r in tqdm(
            p.starmap(
                partial(eval_single_datapoint, args=args),
                zip(
                    test_index[: args.eval_num_examples],
    ...
    logger.info(
        "Num atoms:   Mean: %.3f Median: %.3f"
        % (np.mean(num_atoms), np.median(num_atoms))
    )
    if args.docking_mode in ["vina", "qvina"]:
        ...
        vina_score_only = [
            x["vina"]["score_only"][0]["affinity"] for r in testset_results for x in r
        ]
        vina_min = [
            x["vina"]["minimize"][0]["affinity"] for r in testset_results for x in r
        ]
        logger.info(
        ...
        if args.docking_mode == "vina_full":
            vina_dock = [
                x["vina"]["dock"][0]["affinity"] for r in testset_results for x in r
            ]
            logger.info(
