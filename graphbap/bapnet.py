...
class BAPNet(nn.Module):
    def __init__(
        ...
        self.FusionGraph.append(
            GATConv(graph_dim * 2, graph_dim * 1, GAT_head, concat=False)
        )

        self.OutputLayer = nn.Sequential(
            nn.Linear(graph_dim * 1, graph_dim),
            nn.Hardswish(),
    ...
    def extract_features(
        ...
        complexes_coords = torch.cat([lig_coords, pocket_coords], dim=0).to(
            torch.float32
        )
        complexes_id = torch.LongTensor(
            [0] * lig_coords.shape[0] + [1] * pocket_coords.shape[0]
        ).to(device)
        ...
        pocket_type_emb = torch.cat(
            [pocket_atom_type_emb, pocket_residue_type_emb], dim=1
        )
        ...
        for i in range(self.graph_layers):
            ...
            O_L, lig_coords = LigLayer(
                O_L,
                lig_coords,
                ligand_edge_index,
                node_mask=None,
                edge_mask=None,
                edge_attr=lig_distances,
                update_coords_mask=None,
            )
            O_P, pocket_coords = PocketLayer(
                O_P,
                pocket_coords,
                pocket_edge_index,
                node_mask=None,
                edge_mask=None,
                edge_attr=pocket_distances,
                update_coords_mask=None,
            )
